...
bool CplexSolver::solveCoverUsers(std::vector<int> &B, std::vector<int> &M, int **Y) 
{
	...
	/* populate by row */
	status = populatebyrowCoverUsers(B, M, Y);
...
int CplexSolver::populatebyrowCoverUsers(std::vector<int> &B, std::vector<int> &M, int ** Y)
{
